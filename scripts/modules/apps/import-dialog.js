...
export class ModuleImportDialog extends Dialog {
    ...
    */
	...
	static get requiredSybCoreVersion() {
		return game.modules.get('symbaroum5ecore').version;
	}
	static get requiredDnDCoreVersion() {
		return '2.0.3';
	}
	...
	static neededMigration({ moduleName = this.moduleName, migratedVersionSetting = this.migratedVersionKey, migrationVersions = this.migrationVersions }) {
		...
		const needsMigration = neededMigrationVersions.find((version) => foundry.utils.isNewerVersion(version, lastMigratedVersion));
		logger.debug(
			`${moduleName} ${
				!!needsMigration ? 'requires migration to ' + needsMigration : 'does not require migration'
			} (last migrated version: ${lastMigratedVersion})`
		);
	...
	async updateLastMigratedVersion(version = undefined) {
		if (!version) {
			...
			version = this.migrationVersions.reduce((acc, current) => {
				if (foundry.utils.isNewerVersion(current, acc)) acc = current;
				return acc;
			}, '0.0.0');
		}
	...
	async migrateFolders(
		moduleName,
		moduleTitle,
		adventurePack = this.adventurePack,
		adventurePackName = this.adventurePackName,
		acFolderIDs = this.acFolderIDs
	) {
